 ...
 *
 * $Id: tix.h,v 1.12 2001/02/27 06:41:35 ioilam Exp $
 */
 ...
 *
 * library/pkgIndex.tcl (4 LOC, 2 in comments)
 * unix/README.txt	(example directory name)
 * unix/configure.in	(1 LOC Major, 1 LOC minor)
 * library/Init.tcl	(only if Major.minor changes, not patchlevel) 1 LOC
 * win/makefile.vc	(1 LOC Major, 1 LOC minor, 1 LOC patch)
 * win/makefile.gnu	(1 LOC Major, 1 LOC minor, 1 LOC patch)
 * tests/basic.test	(version checks)
 * tests/README 	(example executable name)
 * tools/tix-man2html.tcl (1LOC)
 * tools/winsetup.iss   (many LOC)
 * docs/SAModule.txt    (example shared lib name)
 */

#define TIX_MAJOR_VERSION   8
#define TIX_MINOR_VERSION   2
#define TIX_RELEASE_LEVEL   TIX_BETA_RELEASE
#define TIX_RELEASE_SERIAL  0

#define TIX_VERSION	    "8.2"
#define TIX_PATCH_LEVEL	    "8.2.0"
...
EXTERN char *           Tix_ZAlloc _ANSI_ARGS_((int nbytes));
